...
def runner():
        ...
        if issuechecker2() == "nocap":
          client.sendMessage(str(bot.channel), "ohunt")
          if not owodes:
            exit()
...
def owopray():
  client.sendMessage(str(bot.channel), "opray")
  print(f"{at()}{bot.color.okgreen} [SENT] {bot.color.reset} opray")
  time.sleep(13)
def gems():
  client.typingAction(str(bot.channel))
  time.sleep(5)
  client.sendMessage(str(bot.channel), "oinv")
  print(f"{at()}{bot.color.okgreen} [SENT] {bot.color.reset} oinv")
  time.sleep(7)
  ...
  if not inv:
    ...
    if '50' in inv:
      client.sendMessage(str(bot.channel), "olb all")
      print(f"{at()}{bot.color.okgreen} [SENT] {bot.color.reset} olb all")
      time.sleep(13)
      gems()
      return
    for item in inv:
    ...
    for level in range(0,3):
      if not len(tier[level]) == 0:
        client.sendMessage(str(bot.channel), "ouse "+str(max(tier[level])))
        print(f"{at()}{bot.color.okgreen} [SENT] {bot.color.reset} ouse {str(max(tier[level]))}")
        time.sleep(7)
